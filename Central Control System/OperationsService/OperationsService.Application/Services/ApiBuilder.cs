...
namespace OperationsService.Application.Services
{
    public class ApiBuilder : IApiBuilder
    {
        ...
        private async Task CheckResponse(HttpResponseMessage? response)
        {
            if (response == null)
            {
                throw new OperationsServiceException(Constants.EmptyResponseException);
            }
            ...
            {
                string res = await response.Content.ReadAsStringAsync();
                throw new OperationsServiceException(string.IsNullOrEmpty(res) ? Constants.DefaultException : res);
            }
        }
    }
}
